...
class BollingerBands(qt.StrategyBase):
    ...
    def on_tick(self, tick_event):
        ...
        # close short position
            ...
            self.adjust_position(symbol, size_from=current_size, size_to=target_size, timestamp=self.current_time)
            print(
                f'{self.current_time}, npv {npv}, CLOSE SHORT ORDER SENT, {symbol}, Price: {current_price:.2f}, MB: {mb[-1]}, Size: {target_size}')
        # close long position
        elif current_price > mb[-1] and current_size > 0:
            target_size = 0
            self.adjust_position(symbol, size_from=current_size, size_to=target_size, timestamp=self.current_time)
            print(
                f'{self.current_time}, npv {npv}, CLOSE LONG ORDER SENT, {symbol}, Price: {current_price:.2f}, MB: {mb[-1]}, Size: {target_size}')
        else:
...
if __name__ == '__main__':
    ...
    datapath = os.path.join(CSV_DATA_DIR, f'{symbol}.csv')
    ...
    if do_optimize:  # parallel parameter search
        ...
        # save to excel
        qt.util.save_one_run_results('{}/output'.format(CSV_DATA_DIR), ds_equity, df_positions, df_trades)

        # ------------------------- Evaluation and Plotting -------------------------------------- #
        strat_ret = ds_equity.pct_change().dropna()
        strat_ret.name = 'strat'
        bm = qt.util.read_ohlcv_csv(os.path.join(CSV_DATA_DIR, f'{benchmark}.csv'))
